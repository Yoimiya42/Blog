...
- 项目结束时，你能独立说清楚：一个功能从需求到上线经过了哪些环节、每个环节为什么这么做、以及当初每个技术选择的取舍是什么。（学习目标，见 1.5）
...
本项目同时是一次完整的软件工程实践演练，覆盖需求工程、架构设计与决策记录、任务拆分与看板、版本控制与代码审查、测试、CI/CD、数据库设计与运维、可观测性、安全自查、维护与演进。

这个目标会**改变一部分决策**：某些「本可以偷懒」的地方要按规范做，因为过程本身就是产出。但它不改变优先级——先上线，再完善。具体的实践清单与引入节奏见 `02-architecture.md` 第 4、5 节。

相应地，本项目的交付物不只是网站，还包括：需求文档、架构文档、ADR 决策记录、技术债台账、OWASP 安全自查报告、CHANGELOG。这些文档本身也是可以拿出去展示的作品。
